...
const Post = ({ id, content, imageUrl, userImageUrl, likes, userEmailAddress, UserName, liked,Timestamp, commentsCount, Comments, isMine, userName }) => {
  ...
  const handleEdit = () => {
    axios.put(`http://localhost:8080/workoutplan/${id}`, { content: postContent }, { withCredentials: true })
      .then(response => {
  ...
  const handleDelete = () => {
    axios.delete(`http://localhost:8080/workoutplan/${id}`, { withCredentials: true })
      .then(response => {

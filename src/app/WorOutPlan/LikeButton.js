...
function LikeButton({ postId, initialLikes, likedStatus }) {
  ...
  const handleLike = () => {
    if (!liked) {
      // Like the post
      axios.get(`http://localhost:8080/workoutplan/like/${postId}`, { withCredentials: true })
        .then(response => {
          setLikes(likes + 1);
          setLiked(true);
        })
      ...
      // Dislike the post
      axios.get(`http://localhost:8080/workoutplan/dislike/${postId}`, { withCredentials: true })
        .then(response => {
          setLikes(likes - 1);
          setLiked(false);
        })
